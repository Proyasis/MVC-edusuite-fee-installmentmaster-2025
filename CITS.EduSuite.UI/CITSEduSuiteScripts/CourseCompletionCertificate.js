var CourseCompletionCertificate = (function () {
        ...
        $("#grid").jqGrid({
           ...
           Resources.Issued,  Resources.IssueDate, Resources.Action],
            colModel: [
                ...
                { key: false, hidden: true, name: 'CourseCompletionCertificateKey', index: 'CourseCompletionCertificateKey', editable: true },
    ...
    function editLink(cellValue, options, rowdata, action) {
        ...
        if (!rowdata.CourseCompletionCertificateKey) {
            return '<div class="divEditDelete"><a  class="btn btn-outline-warning btn-sm ml-3" onClick="CourseCompletionCertificate.CourseCompletionCertificatePopup(' + rowdata.CourseCompletionCertificateKey + ',' + rowdata.RowKey +')"><i class="fa fa-share" aria-hidden="true"></i>' + Resources.Issue + '</a></div>';

        }
        else {
            return '<div class="divEditDelete"><a  class="btn btn-outline-warning btn-sm ml-3" onClick="CourseCompletionCertificate.CourseCompletionCertificatePopup(' + rowdata.CourseCompletionCertificateKey + ',' + rowdata.RowKey + ')"><i class="fa fa-share" aria-hidden="true"></i>' + Resources.Issue + '</a>' + '<a class="btn btn-outline-danger btn-sm ml-2"  onclick="javascript:deleteCourseCompletionCertificate(' + rowdata.CourseCompletionCertificateKey + ');return false;" >  <i class="fa fa-trash pointer" aria-hidden="true"></i></a>' + '<a  class="btn btn-outline-primary btn-sm ml-2" onClick="CourseCompletionCertificate.PrintCourseCompletionCertificate(' + rowdata.RowKey + ')"><i class="fa fa-print" aria-hidden="true"></i></a> </div>';
    ...
    var CourseCompletionCertificatePopup = function (Id, ApplicationKey) {
        var obj = {};
        obj.ApplicationKey = ApplicationKey;
        obj.Id = Id;
        var url = $("#hdnAddEditCourseCompletionCertificate").val() + "?" + $.param(obj);
    ...
    var fetchCourseCompletionCertificate = function () {
        var obj = {};
        obj.ApplicationKey = $("#ApplicationKey").val();
        $("#dvFetchCourseCompletionCertificate").mLoading();

        $.ajax({
            type: "Get",
            url: $("#hdnFetchCourseCompletionCertificateDetails").val(),
            data: obj,
            ...
            success: function (data) {
                ...
                $.ajax({
                    type: 'GET',
                    crossDomain: true,
                    url: $("#hdnCourseCompletionCertificatePath").val() + "?no-cache=" + new Date().getTime(),
                    success: function (response) {
                        ...
                        $('#dvFetchCourseCompletionCertificate').html(html);
                    },
                    error: function (xhr) {

                    },
                    complete: function () {
                        $("#dvFetchCourseCompletionCertificate").mLoading("destroy");
                    }
                })
    ...
    function formSubmit() {
        ...
        var $form = $("#frmAddEditCourseCompletionCertificate")
        ...
        if ($form.valid()) {
            ...
            if (typeof response == "string") {
                ...
                var successmsg = "Course Completion Certificate Issued Successfully";
                $.alert({
                    type: 'green',
                    ...
                    buttons: {
                        Ok: {
                            ...
                            action: function () {
                                window.location.href = $("#hdnCourseCompletionCertificateList").val();
                            }
                        }
                    }
                })
    ...
    var printCourseCompletionCertificate = function (Id, rebind) {
        var obj = {};
        ...
        if (obj.ApplicationKey) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    var url = $("#hdnCourseCompletionCertificateDefaultPath").val() + "?no-cache=" + new Date().getTime();
    ...
    return {
        ...
        FetchCourseCompletionCertificate: fetchCourseCompletionCertificate,
        CourseCompletionCertificatePopup: CourseCompletionCertificatePopup,
        PrintCourseCompletionCertificate: printCourseCompletionCertificate
...
function deleteCourseCompletionCertificate(rowkey) {
    var result = EduSuite.Confirm({
        title: Resources.Confirmation,
        content: Resources.Delete_Confirm_Application,
        actionUrl: $("#hdnDeleteCourseCompletionCertificate").val(),
        actionValue: rowkey,
