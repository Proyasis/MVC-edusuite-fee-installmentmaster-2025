...
(function ($) {
    ...
    var TagSuggest = function (element, options) {
        ...
        this.addToSelection = function (items, isSilent) {
            if (!cfg.maxSelection || _selection.length < cfg.maxSelection) {
                ...
                $.each(items, function (index, json) {
                    if (cfg.allowDuplicate || $.inArray(json[cfg.valueField], ms.getValue()) === -1) {
        ...
        this.setValue = function (data) {
            ...
            var checkArray = $.map(_cbData, function (n, i) {
                return n[cfg.valueField];
            });
            $.each(values, function (i) {
                var obj = _cbData.filter(function (n, p) {
                    return n[cfg.valueField] == values[i];
                })[0];
                if (obj) {
