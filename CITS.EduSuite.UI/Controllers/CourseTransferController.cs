...
{
    public class CourseTransferController : BaseController
    {
        private ICourseTransferService CourseTransferService;
        ...
        public CourseTransferController(ICourseTransferService objCourseTransferService, ISharedService objSharedService,
                  ISelectListService objselectListService
                  , INotificationTemplateService objnotificationTemplateService)
        {
            this.CourseTransferService = objCourseTransferService;
        ...
        [ActionAuthenticationAttribute(MenuCode = MenuConstants.CourseTransfer, ActionCode = ActionConstants.View)]
        [HttpGet]
        public ActionResult CourseTransferList()
        {
        ...
        public JsonResult GetApplications(string ApplicantName, string MobileNumber, short? BranchKey, long? CourseKey, short? UniversityKey, short? BatchKey, string sidx, string sord, int page, int rows)
        {
            ...
            applicationList = CourseTransferService.GetApplications(objViewModel, out TotalRecords);
        ...
        [ActionAuthenticationAttribute(MenuCode = MenuConstants.CourseTransfer, ActionCode = ActionConstants.AddEdit)]
        public ActionResult AddEditCourseTransfer(long? id, long? ApplicationKey)
        {
            ...
            model = CourseTransferService.GetCourseTransferById(model);
            if (model == null)
            {
                model = new CourseTransferViewModel();
            }
        ...
        [HttpPost]
        public ActionResult AddEditCourseTransfer(CourseTransferViewModel model)
        {
            if (ModelState.IsValid)
            {
                if (model.RowKey == 0)
                {
                    model = CourseTransferService.CreateCourseTransfer(model);
                }
                else
                {
                    model = CourseTransferService.UpdateCourseTransfer(model);
        ...
        [ActionAuthenticationAttribute(MenuCode = MenuConstants.CourseTransfer, ActionCode = ActionConstants.Delete)]
        public ActionResult DeleteCourseTransfer(long Id)
        {
            CourseTransferViewModel objviewModel = new CourseTransferViewModel();
            try
            {
                objviewModel = CourseTransferService.DeleteCourseTransfer(Id);
            }
            catch (Exception)
            {
        ...
        public void SendNotificationInBackground(CourseTransferViewModel model)
        {
        ...
        private void SendNotification(Object model)
        {
            ...
            CourseTransferViewModel objViewModel = (CourseTransferViewModel)model;
            ...
            objNOtificationViewModel.RowKey = objViewModel.ApplicationKey ?? 0;
        ...
        {
            CourseTransferViewModel objViewModel = new CourseTransferViewModel();
            objViewModel.ToAcademicTermKey = AcademicTermKey ?? 0;
            CourseTransferService.GetCourseType(objViewModel);
            return Json(objViewModel.CourseTypes, JsonRequestBehavior.AllowGet);
        }

        [HttpGet]
        public JsonResult GetCourseByCourseType(short? CourseTypeKey, short? AcademicTermKey,long? FromCourseKey,short? FromAcademicTermKey)
        {
            CourseTransferViewModel objViewModel = new CourseTransferViewModel();
            objViewModel.CourseTypeKey = CourseTypeKey ?? 0;
            objViewModel.ToAcademicTermKey = AcademicTermKey ?? 0;
            objViewModel.FromAcademicTermKey = FromAcademicTermKey ?? 0;
            objViewModel.FromCourseKey = FromCourseKey ?? 0;
            CourseTransferService.GetCourseByCourseType(objViewModel);
        ...
        {
            CourseTransferViewModel objViewModel = new CourseTransferViewModel();
            objViewModel.ToCourseKey = CourseKey ?? 0;
            objViewModel.ToAcademicTermKey = AcademicTermKey ?? 0;
            CourseTransferService.GetUniversity(objViewModel);
        ...
        public JsonResult GetStudentTotalFeeDetails(string id)
        {
            ...
            var CourseTransfer = CourseTransferService.BindAllCourseTansferDetailsById(objViewModel);
            var jsonData = new
            {
                rows = CourseTransfer
            };
