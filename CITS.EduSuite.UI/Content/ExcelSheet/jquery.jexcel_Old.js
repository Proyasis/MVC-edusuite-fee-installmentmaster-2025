...
(function ($) {
    ...
    var methods = {
        ...
        init: function (options) {
            ...
            if (options.csv) {
                if (!$.csv) {
                    ...
                    // var workbook = XLSX.readFile(options.csv);
            ...
            if (!$('.jexcel_corner').length) {
                ...
                $(document).on('mouseup', function (e) {
                    if (e.target.id == 'jexcel_arrow') {
                        ...
                        var current = $('#' + $.fn.jexcel.current);
                        if ($(selection).length > 0) {

                            $($(selection[0]).closest("div")).append(divOption)
                            // First and last cells
                            var o = $(selection[0]).prop('id').split('-');
                            var d = $(selection[selection.length - 1]).prop('id').split('-');

                            var main = $(selection[0]).closest("div");
                            var copyOptions = [{ "Id": "1", "Name": "Copy" }, { "Id": "2", "Name": "Fill Series" }];
                            var divOption = $("<div/>");
                            $(divOption).addClass("copy-option")

                            var px = parseInt(o[0]);
                            var ux = parseInt(d[0]);
                            var py = parseInt(o[1]);
                            var uy = parseInt(d[1]);
                            var data = $(main).jexcel('getData', true);
                            data = data[0][0];
                            if (((px == ux) || (py == uy)) && ((parseFloat(data) == data || data.toString().substr(0, 1) == '='))) {
                                var mouseX;
                                var mouseY;
                                mouseX = e.pageX;
                                mouseY = e.pageY;
                                $(divOption).css({ 'top': mouseY, 'left': mouseX })
                                $.each(copyOptions, function () {
                                    $(divOption).append(
                                        $('<label />', {
                                            'text': this.Name
                                        }).append(
                                ...
                                $(current).jexcel('copyData', o, d);
                            }

                            $(divOption).find("input").change(function (e) {
                                if ($(this).val() == "2") {
                                    $(current).jexcel('fillSeries', o, d);
                            ...
                            $($(selection[0]).closest("table")).on("click", function (e) {
                                if ($(divOption) != e.target) {
                                    if ($(divOption)) {
                                        $(divOption).remove()
                            ...
                            if (e.which == 37) {
                                ...
                                if ($($.fn.jexcel.selectedCell).hasClass('edition')) {
                                    ...
                                    if ($.fn.jexcel.defaults[$.fn.jexcel.current].columns[columnId[0]].type == 'calendar') {
                                        $('#' + $.fn.jexcel.current).find('editor').datepicker('hide');
                                    } else {
            ...
            if ($(cell).hasClass('readonly') == true) {
                ...
                if (options.columns[position[0]].editor) {
                    ...
                    if (options.columns[position[0]].type == 'checkbox' || options.columns[position[0]].type == 'hidden') {
                        ...
                        //options.columns[position[0]].option.onClose = function () {
                        //    $(main).jexcel('closeEditor', $(cell), true);
                        //}

                        // Current value
                        $(editor).attr("data-inputmask", "'alias': 'date'");
                        $(editor).datepicker(options.columns[position[0]].option).on('hide', function (e) {
                            $(main).jexcel('closeEditor', $(cell), true);
                        });
                        $(editor).inputmask();
                        if (value) {
                            ...
                            if (result) {
                                ...
                                $(editor).datepicker('setDate', today);
                            }
                        ...
                        $(editor).focus(function () {
                            ...
                            if ($(this).val().charAt(0) != "=") {
                                var newVal = $(this).val();
                                if (options.columns[position[0]].editorRegx)
                                    newVal = newVal.replace(options.columns[position[0]].editorRegx, "");
                                $(this).val(newVal);
                            }
                            else {
        ...
        updateCell: function (v, force) {
            ...
            if (options.columns[position[0]].editor) {
                ...
                if (options.columns[position[0]].type == 'checkbox') {
                    ...
                    val = '';
                    //if (value != 'undefined') {
                    //    val =$.fn.datepicker('setDate', value); //$.fn.jcalendar('label', value, options.columns[position[0]].options.format);
                    //} else {
                    //    val = '';
                    //}
                    $(v.cell).html('<input type="hidden" value="' + value + '">' + value);
                } else {
                    if (value) {
                        if (value.substr(0, 1) == '=') {
                    ...
                    if (value == parseFloat(value) && value.toString().split('.').length > 1)
        ...
        updateCornerPosition: function () {
            ...
            if ($(cells).length && !$(cells).hasClass("readonly")) {
                corner = $(cells).last();
        ...
        getHeaderData: function () {
            // Get row
            row = $(this).find("thead").find('tr').parent().find('td').not(':first');
            ...
            if (row.length > 0) {
                for (i = 0; i < row.length; i++) {
                    str.push($(this).jexcel('getHeader', i));
                }
            }
        ...
        getData: function (highlighted) {
            ...
            for (j = 0; j < y; j++) {
                ...
                for (i = 0; i < x; i++) {
                    ...
                    if (cell.length > 0 && (!highlighted || $(cell).hasClass('highlight'))) {
                        // Get value
                        if (!dataset[py]) {
                            dataset[py] = [];
                        }
        ...
        getFormulaData: function (highlighted) {
            // Control vars
            var dataset = [];
            var px = 0;
            var py = 0;
            ...
            for (j = 0; j < y; j++) {
                ...
                for (i = 0; i < x; i++) {
                    ...
                    if (!highlighted || $(cell).hasClass('highlight')) {
                        ...
                        dataset[py][px] = $(this).jexcel('getFormula', $(cell));
                        px++;
                    }
                }
                if (px > 0) {
        ...
        */
            ...
            if (options.allowInsertColumn == true) {
                ...
                for (i = num; i < (num + numColumns) ; i++) {
                    ...
                    $.each(tr, function (k, v) {
                        if (k == 0) {
                            options.data[k][i] = properties.header;
                        }
                        else {
                            // Update data array
                            options.data[k][i] = '';
                        ...
                        $(v).append(td);
                        var formula = options.data[k][i - 1]
                        var prevHeader = $(main).jexcel('getHeader', i - 1);
                        var prevHeader1 = $(main).jexcel('getHeader', i - 2);
                        var currHeader = $(main).jexcel('getHeader', i);
                        if (formula.substr(0, 1) == '=') {
                            options.data[k][i] = formula.replace(prevHeader, currHeader).replace(prevHeader, currHeader).replace(prevHeader1, prevHeader);
                        }
                        // Cell data
                        records.push({
                            cell: $(td),
                            newValue: options.data[k][i].replace(),
                            oldValue: '',
                        });
                    });
                }
            ...
            if (options.allowDeleteRow == true) {
                ...
                if (options.data.length > 1) {
                    ...
                    if (parseInt(lineNumber) > -1) {
                        ...
                        var row = $("#" + id + " td#row-" + lineNumber).closest("tr");
                        var currData = $(this).jexcel('getRowData', lineNumber);
                        if (typeof (options.ondeleterow) == 'function') {
                            options.ondeleterow($(this), currData);
                        }
                        $(row).remove();
                    ...
                    // After changes
                    $(this).jexcel('afterChange');
                    if (typeof (options.onafterdeleterow) == 'function') {
                        options.onafterdeleterow($(this), currData);
                    }
                } else {
        ...
        deleteMultipleRow: function (lineNumbers) {
            ...
            if (options.data.length > 1) {
                // Remove from source
                $.fn.jexcel.defaults[id].data = $.fn.jexcel.defaults[id].data.filter(function (n, p) {
                    return lineNumbers.indexOf(n.toString()) === -1;
                });
                var currData = $.fn.jexcel.defaults[id].data;
        ...
        copyData: function (o, d) {
            ...
            if (py == uy) {
                var data = $(this).jexcel('getValue', (px - 1) + "-" + py);
                var formula = $(this).jexcel('getFormula', (px - 1) + "-" + py);
            }
            else {
                var data = $(this).jexcel('getValue', px + "-" + (py - 1));
                var formula = $(this).jexcel('getFormula', px + "-" + (py - 1));
            }
            ...
            for (j = py; j <= uy; j++) {
                ...
                for (i = px; i <= ux; i++) {
                    ...
                    oldData = $(this).jexcel('getValue', $(cell));
                    var oldFomula = $(this).jexcel('getFormula', $(cell).prop("id"));
                    if (oldFomula) {
                        if (oldFomula.substr(0, 1) == '=') {
                            oldData = oldFomula
                        }
                    }
                    // Update non-readonly
        ...
        fillSeries: function (o, d) {
            ...
            for (j = py; j <= uy; j++) {
                ...
                for (i = px; i <= ux; i++) {
                    ...
                    var prevHeader = $(this).jexcel('getHeader', i - 1);
                    var prevHeader1 = $(this).jexcel('getHeader', i - 2);
                    var currHeader = $(this).jexcel('getHeader', i);

                    if (formula) {
                        if (formula.substr(0, 1) == '=') {

                            formula = newData = formula.replace(prevHeader, currHeader).replace(prevHeader, currHeader).replace(prevHeader1, prevHeader)
                        }
                        else if (parseFloat(newData) == newData) {
                            newData = (parseFloat(newData) + 1).toString()
                        }
        ...
        formula: function () {
            ...
            $.each(variables, function (k, v) {
                ...
                $(main).jexcel('applyFormula', k, $(v).prop('id'));
                v = $(main).jexcel('getValue', $(v));
                if (v == parseInt(v)) {
                    window[i] = parseInt(v);
                }
                else if (v == parseFloat(v)) {
                    window[i] = parseFloat(v);
                }
                else {
                    window[i] = v;
                }
            ...
            // value = '<input type="hidden" value="' + formula + '">' +'<span>'+value+'</span>';
            // // Update cell content
            // $(main).find('#' + column).html(value);
            // }
            // } else {
        ...
        applyFormula: function (k, column) {
            var main = $(this);
            var id = $(this).prop('id');
            var columns = $.fn.jexcel.defaults[id].dynamicColumns;
            formula = $(main).jexcel('getFormula', column);
            // Column value is a formula
            if (formula) {
                if (formula.substr(0, 1) == '=') {
                    // Convert formula to javascript
        ...
        download: function () {
            ...
            pom.href = url;
            pom.setAttribute('download', 'jexcelTable.xls');
            pom.click();
        },
